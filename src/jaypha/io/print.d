...
void print(Writer,S...)(Writer w, S args)
{
  foreach (arg; args)
  {
    ...
    else static if (__traits(compiles,arg.toString()))
    {
      stream.put(arg.toString());
    }
    else
    {
      // Most general case
...
void fprintln(Writer,Char, A...)(Writer w, in Char[] fmt, A args)
{
  std.format.formattedWrite(w, fmt, args);
  w.put('\n');
}
...
unittest
{
  ...
  napp.fprint("b%smat",d);
  assert(napp.data == "ab5truegxyz\nbformat");
  napp.fprintln("a %d format",10);
  assert(napp.data == "ab5truegxyz\nbformata 10 format\n");
  napp.println();
  assert(napp.data == "ab5truegxyz\nbformata 10 format\n\n");
}
